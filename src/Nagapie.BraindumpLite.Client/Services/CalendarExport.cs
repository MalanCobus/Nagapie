...
public static class CalendarExport
{
    ...
    public static string Create(string title, string category, DateTime start, DateTime end, bool allDay)
    {
        if (end <= start)
        {
            throw new ArgumentException("End must follow start.");
        }
        ...
        var lines = new List<string>
        {
            "BEGIN:VCALENDAR",
            "VERSION:2.0",
            "PRODID:-//Nagapie//Braindump Lite//EN",
            "CALSCALE:GREGORIAN",
            "BEGIN:VEVENT",
            $"UID:{Guid.NewGuid()}@nagapie",
        ...
        {
            lines.Add($"DTSTART:{start.ToUniversalTime():yyyyMMddTHHmmssZ}");
            lines.Add($"DTEND:{end.ToUniversalTime():yyyyMMddTHHmmssZ}");
        }

        lines.Add("SUMMARY:" + Escape(title));
        lines.Add("DESCRIPTION:" + Escape(category));
